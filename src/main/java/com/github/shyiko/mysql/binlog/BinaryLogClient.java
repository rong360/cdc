...
public class BinaryLogClient implements BinaryLogClientMXBean {
	...
	private final static Logger logger = LoggerFactory.getLogger(BinaryLogClient.class);
	...
	private long keepAliveConnectTimeout = TimeUnit.SECONDS.toMillis(10);
	...
	public boolean isMainThreadFinish() {
		return mainThreadFinish;
	}

	public void setMainThreadFinish(boolean mainThreadFinish) {
		this.mainThreadFinish = mainThreadFinish;
	}
	...
	public void connect() throws IOException {
		...
		try {
			String cacheFileName = EtcdApi.get(RongUtil.getBinlogFileKey());
			String cacheFilePos = EtcdApi.get(RongUtil.getBinlogPosKey());
			if(cacheFileName != null && cacheFileName != "" && cacheFilePos !=null && cacheFilePos !=""){
				this.setBinlogFilename(cacheFileName);
				this.setBinlogPosition(Long.valueOf(cacheFilePos));
			}
			establishConnection();
			...
			//getDefaultTimeZone();//获取时区 CST时区存在歧义,不适用
				...
				while (true) {
					...
					try {
						...
						try {
							...
							logger.warn("Send ping command failed :" + e.getMessage() + "Trying to restore lost connection to " + hostname + ":" + port);  
							try {
								if (isConnected()) {
									disconnectChannel();
								}
								logger.warn("begin to connection to mysql again:" + String.valueOf(++maxretry) + " times");
								connect(keepAliveConnectTimeout);
							} catch (Exception ce) {
								logger.warn("Failed to restore connection to " + hostname + ":" + port +
										". Next attempt in " + keepAliveInterval + "ms" + ce.getMessage());

							}
						}
					} finally {
						shutdownLock.unlock();
						if(maxretry > GlobalConfig.ping_failed_maxretry){
							logger.warn("spawn thread exceed max retry:" + String.valueOf(maxretry) + "times,break!");
	...
	private void listenForEventPackets() throws IOException {
		...
		try {
			while (inputStream.peek() != -1  && !this.isMainThreadFinish()) {
				...
				if (gtidSet != null && eventType == EventType.GTID) {
					if (previousGtidEvent != null) {
						if (advanceGTID()) {
							...
							logger.warn( "GtidSet wasn't synchronized before GTID. " +
